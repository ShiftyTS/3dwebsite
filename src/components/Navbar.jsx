    ...
    <nav className={`${styles.paddingX} w-full flex py-5 items-center fixed top-0 bg-primary z-20`}>
      <div className='w-full justify-between items-center flex max-w-7x1 mx-auto'>
        ...
        <div className='sm:hidden flex flex-1 items-center justify-end'>
          <img src={toggle ? close : menu} alt="hamburger" className='w-[28px] h-[28px] cursour-pointer object-contain' onClick={() => setToggle(!toggle)} />
            ...
            <ul className='list-none flex items-start flex-col justify-end gap-4'>
              {navLinks.map((link) => (
                <li
                  key={link.id}
                  className={`${active === link.title ? "text-white" : "text-secondary"
                    } text-[16px] hover:text-white font-poppins font-medium cursor-pointer`}
                  onClick={() => {
